...
public class RabbitMQConfig {
    ...
    public Queue asyncQueue() {
        ...
        // 当所有消费客户端连接断开后，是否自动删除队列
        boolean autoDelete = false;
        return new Queue(rabbitMQProperties.getAsyncQueueName(), durable, exclusive, autoDelete);
    }
    ...
    @Bean
    public Binding bindingAsyncQueue() {
        return BindingBuilder.bind(notifyQueue())
                .to(directExchange())
                .with(rabbitMQProperties.getAsyncQueueName());
    }
